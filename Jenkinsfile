    ...
    stage('Auth & Docker Build') {
      steps {
        withCredentials([file(credentialsId: 'gcp-jenkins', variable: 'GCLOUD_KEY')]) {
          ...
          docker build -t europe-west3-docker.pkg.dev/$PROJECT_ID/my-docker-repo2/myapp:$IMAGE_TAG .
          docker push europe-west3-docker.pkg.dev/$PROJECT_ID/my-docker-repo2/myapp:$IMAGE_TAG
          """
        }
      }
    }
